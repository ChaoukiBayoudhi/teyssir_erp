...
class StockTakeTests(TestCase):
    def test_post_stocktake_posts_variance_adjustment(self):
        p = Product.objects.create(sku="X", name_fr="Cahier", qty_on_hand=10)
        result = post_stocktake([{"product_id": p.id, "counted_qty": "8"}], terminal="C1")

        p.refresh_from_db()
        self.assertEqual(p.qty_on_hand, 8)          # cached on-hand corrected
        self.assertEqual(result["adjusted"], 1)
        self.assertEqual(result["lines"][0]["variance"], "-2")
        mv = StockMovement.objects.get(reason="STOCKTAKE")
        self.assertEqual(mv.qty, -2)                # audit trail of the correction

    def test_no_movement_when_count_matches(self):
        p = Product.objects.create(sku="Y", name_fr="Stylo", qty_on_hand=5)
    ...
    def test_stocktake_enqueues_and_rolls_up_at_hub(self):
        ...
        apply_movement(product_id=p.id, qty=10, reason=StockMovement.RECEIPT)  # ledger +10
        ...
        p.refresh_from_db()
        self.assertEqual(p.qty_on_hand, 8)   # 10 received - 2 stock-take
